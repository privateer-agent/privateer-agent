...
import { ModeGate, type AskOutcome } from "../src/permissions/modeGate.ts";
...
// Ported verbatim from tree-cli/tests/permissions.test.ts (only import paths
// changed: uiGate → modeGate, config/schema → config/permissionMode). Asserts the
// pure mode policy + the ModeGate engine (local, remote, no-quarter branches).
...
test("no-quarter remote turn still respects a hard plan-mode deny", async () => {
  const g = makeGate("plan", "allow", true, true);
  assert.equal(await g.gate.request(edit), "deny");
  assert.equal(g.asks(), 0);
});
